...
from subjects import SubjectsForm
from teachers import TeachersForm
...
#to catch the 2 dimensional list and put it in a multiple entries or the grid
    ...
    for i in range(len(lst)):
            for j in range(len(lst[0])):
                    mgrid = tk.Entry(studentsWindow, width=25)
...
#to clear all of the text entries for rows that is greater than 6
    if n==1:
            for label in studentsWindow.grid_slaves():
...
def save():
    ...
    if r==True:
        ...
        id=newid+1
        studentid.set(id)
        mydict = {"studid":int(float(studid.get())), "studname": studname.get(),"studemail":studemail.get(), "studcourse": studcourse.get()} 
...
def delete():
    ...
    if r==True:
        ...
        myquery = {"studid":int(float(studid.get()))}
...
#Students GUI Form
studentsWindow = tk.Tk()
studentsWindow.title("Students Form")
studentsWindow.geometry("1050x400")
studentsWindow.configure(bg="pink")

#Menu Bar 
menubar = tk.Menu(studentsWindow)
filemenu = tk.Menu(menubar, tearoff=0)
menubar.add_cascade(label="File", menu=filemenu)
filemenu.add_command(label="Subjects", command=SubjectsForm.sujectform)
filemenu.add_command(label="Teachers", command=TeachersForm.teacherform)
filemenu.add_separator()
filemenu.add_command(label="Close", command=studentsWindow.quit)

#Edit Menu
editmenu=tk.Menu(menubar, tearoff=0)
menubar.add_cascade(label="Edit", menu=editmenu)
editmenu.add_command(label="Undo",command=SubjectsForm.sujectform)
editmenu.add_separator()
editmenu.add_command(label="Cut", command=TeachersForm.teacherform)

studentsWindow.config(menu=menubar)

#Students Enlisment Form GUI
label = tk.Label(studentsWindow,text="Students Enlistment Form", width=30, height=1, bg="white", anchor="center")
label.config(font=("Courier",10))
label.grid(column=2,row=1)
#Student ID
label = tk.Label(studentsWindow,text="Student ID:", width=10, height=1, bg="white")
label.grid(column=1,row=2)

studentid=tk.StringVar()
studid=tk.Entry(studentsWindow, textvariable=studentid)
studid.grid(column=2, row=2)
studid.configure(state=tk.DISABLED)
#Student Name
label = tk.Label(studentsWindow,text="Student Name:", width=15, height=1, bg="white")
label.grid(column=1,row=3)

studentname=tk.StringVar()
studname=tk.Entry(studentsWindow, textvariable=studentname)
studname.grid(column=2, row=3)
#Student Email
label = tk.Label(studentsWindow,text="Student Email:", width=15, height=1, bg="white")
label.grid(column=1,row=4)

studentemail=tk.StringVar()
studemail=tk.Entry(studentsWindow, textvariable=studentemail)
studemail.grid(column=2, row=4)
#Student Course
label = tk.Label(studentsWindow,text="Student Course:", width=15, height=1, bg="white")
label.grid(column=1,row=5)

studentcourse=tk.StringVar()
studcourse=tk.Entry(studentsWindow, textvariable=studentcourse)
